...
| AVR Port | AVR Pin | Arduino Pin | HUB75 Display | Description | 
| --- | --- | --- | --- | --- |   
...
If you need to reassign pins for the HUB75 display, you'll end up editing `panel_impl.h` and `panel.cpp`.  If you want to reassign the pins used for `R1`, `G1`, `B1`, `R2`, `G2`, or `B2` you're going to end up rewriting the `update()` method in such a way that each update to the panel will take more time.
...
### Structs
There are two structures used by the library:  the `Point` and the `Rect` structs.  `Point` defines a point in terms of `X/Y` coordinates, and `Rect` defines a rectangular region defined by the four corner `X/Y` coordinates:
```   
typedef struct
{
  int16_t x;
